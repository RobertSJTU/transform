...
var BtyTest = []testBty {
  testBty {[]byte{194, 125, 101, 129, 185, 39, 133, 131, 75, 56, 31, 166, 151, 196, 176, 255, 196, 87, 75, 73, 87, 67, 114, 46, 10, 203, 118, 1, 177, 182, 139, 153}, 
  ...
  testBty {[]byte{210, 6, 195, 255, 97, 203, 104, 69, 146, 219, 13, 40, 27, 9, 17, 177, 78, 4, 219, 226, 180, 102, 112, 44, 157, 12, 8, 121, 191, 109, 112, 110}, 
  ...
  testBty {[]byte{24,243,177,145,1,158,131,135,138,129,85,122,190,187,42,253,161,153,227,29,34,225,80,216,191,77,244,86,22,113,190,108},
...
func TestPrivKeyToPub(t *testing.T) {
  ...
  t.Run("test the correct result", func(t *testing.T){
    for _, v := range(BtyTest) {
    pubTest, err := bty.PrivKeyToPub(v.priv)
    require.Nil(err)
    require.Equal(pubTest, v.pub)
    }      
    })
  t.Run("test the err", func(t *testing.T){
    _, err := bty.PrivKeyToPub(errorPriv)
...
func TestPubkeyToAddress(t *testing.T) {
  ...
  t.Run("test the correct result", func(t *testing.T){
    for _, v := range(BtyTest) {
    addrTest, err:= bty.PubKeyToAddress(v.pub)
    require.Nil(err)
    require.Equal(addrTest, v.address)
    }
  })
  t.Run("test the err", func(t *testing.T){
    _, err1 := bty.PubKeyToAddress(errorPub1)
    _, err2 := bty.PubKeyToAddress(errorPub2)
...
func BenchmarkPrivKeyToPub(b *testing.B) {
  for i := 0; i < b.N; i++ {
    bty.PrivKeyToPub(BtyTest[1].priv)
  }
}
func BenchmarkPubkeyToAddress(b *testing.B) {
  for i := 0; i < b.N; i++ {
    bty.PubKeyToAddress(BtyTest[1].pub)
  }
}
