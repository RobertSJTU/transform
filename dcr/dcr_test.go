...
var DcrTest = testDcr {[]byte {78, 25, 183, 99, 11, 198, 222, 40, 11, 126, 6, 190, 214, 93, 228, 151, 24, 166, 90, 196, 142, 60, 150, 40, 65, 219, 131, 71, 158, 57, 76, 100}, 
           []byte{2, 168, 244, 8, 9, 195, 187, 36, 245, 88, 13, 68, 125, 124, 191, 73, 83, 232, 19, 101, 108, 243, 122, 150, 89, 251, 221, 201, 183, 184, 38, 96, 78},
           "Dsa8ijAfczHZYDPyEgvWHA3k5ixaiZKiHQy"}
...
func TestPrivKeyToPub(t *testing.T) {
  ...
  t.Run("test the correct result", func(t *testing.T){
    pubTest, err := dcr.PrivKeyToPub(DcrTest.priv)
    require.Nil(err)
    require.Equal( DcrTest.pub, pubTest)      
    })
  t.Run("test the err", func(t *testing.T){
    _, err := dcr.PrivKeyToPub(errorPriv)
...
func TestPubkeyToAddress(t *testing.T) {
  require := require.New(t)
  t.Run("test the correct result", func(t *testing.T){
    addrTest, err:= dcr.PubKeyToAddress(DcrTest.pub)
    require.Nil(err)
    require.Equal(DcrTest.address, addrTest)
  })
  t.Run("test the err", func(t *testing.T){
    _, err1 := dcr.PubKeyToAddress(errorPub1)
    _, err2 := dcr.PubKeyToAddress(errorPub2)
...
func BenchmarkPrivKeyToPub(b *testing.B) {
  for i := 0; i < b.N; i++ {
    dcr.PrivKeyToPub(DcrTest.priv)
  }
}
func BenchmarkPubkeyToAddress(b *testing.B) {
  for i := 0; i < b.N; i++ {
    dcr.PubKeyToAddress(DcrTest.pub)
  }
}
