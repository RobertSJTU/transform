...
var EthTest = testEth {[]byte {40, 156, 40, 87, 212, 89, 142, 55, 251, 150, 71, 80, 126, 71, 163, 9, 214, 19, 53, 57, 191, 33, 168, 185, 203, 109, 248, 143, 213, 35, 32, 50}, 
...
func TestPrivKeyToPub(t *testing.T) {
  ...
  t.Run("test the correct result", func(t *testing.T){
    pubTest, err := eth.PrivKeyToPub(EthTest.priv)
    require.Nil(err)
    require.Equal( EthTest.pub, pubTest)      
    })
  t.Run("test the err", func(t *testing.T){
    _, err := eth.PrivKeyToPub(errorPriv)
...
func TestPubkeyToAddress(t *testing.T) {
  require := require.New(t)
  t.Run("test the correct result", func(t *testing.T){
    addrTest, err:= eth.PubKeyToAddress(EthTest.pub)
    require.Nil(err)
    require.Equal(EthTest.address, addrTest)
  })
  t.Run("test the err", func(t *testing.T){
    _, err1 := eth.PubKeyToAddress(errorPub1)
    _, err2 := eth.PubKeyToAddress(errorPub2)
...
func BenchmarkPrivKeyToPub(b *testing.B) {
  for i := 0; i < b.N; i++ {
    eth.PrivKeyToPub(EthTest.priv)
  }
}
func BenchmarkPubkeyToAddress(b *testing.B) {
  for i := 0; i < b.N; i++ {
    eth.PubKeyToAddress(EthTest.pub)
  }
}